...
@login_required
@post_job_routes.route("/company/create_job", methods=['POST'])
def create_post_job():
        session = None
        try:
            data = request.json
            ...
            NewJob = Post_job(company_id=current_user.id, job_name=job_name, job_description=job_description, post_until=post_until, qualification=qualification, benefit=benefit, address_job=address_job, job_level=job_level, job_category=job_category, vacancy=vacancy, educational_requirenment=educational_requirenment)
...
#     post_job_data = []
#     for post_job in current_user.post_job:
#         post_job_data.append({
